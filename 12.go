...
func simstep() bool {
	...
	if len(newstate) == len(state) {
		...
		if equal {
			fmt.Printf("automata stalled %d (%d)\n", count(), startid)
			stalled = true
		}
	}
	state = newstate
...
func main() {
	...
	//const NGEN = 20
	const NGEN = 50000000000
	//const NGEN = 160

	for gen := 0; gen < NGEN; gen++ {
		if gen%10000 == 0 {
			fmt.Printf("progress %d/%d %g\n", gen, NGEN, 100*float64(gen)/NGEN)
		}
		fmt.Printf("%d %s\n", gen, string(state))
		stalled := simstep()
		if stalled {
			startid += NGEN - gen - 1
			fmt.Printf("final count: %d (%d)\n", count(), startid)
			return
		}
	}
