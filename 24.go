...
func main() {
	...
	for i := range ImmuneSystem.groups {
		ImmuneSystem.groups[i].attackPower += boost
	}
...
const debugPrintStatus = false
const debugTargetSelection = false
...
// run with:
// for i in $(seq 1 100); do go run 24.go $i; done
